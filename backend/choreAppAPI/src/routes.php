...
$app->put('/tasks/edit/[{taskID}]', function($request, $response, $args){
    ...
    $sql="UPDATE Tasks set status=:status where taskID=:taskID;
        UPDATE TaskDetails set taskRating=:taskRating where taskID=:taskID";
    $sth=$this->db->prepare($sql);
...
$app->put('/taskDetails/edit/[{taskID}]', function($request, $response, $args){
    $input=$request->getParsedBody();
    $sql="UPDATE TaskDetails set taskRating=:taskRating, taskAward=:taskAward, taskTitle=:taskTitle, taskDescript=:taskDescript, deadline=:deadline where taskID=:taskID";
    $sth=$this->db->prepare($sql);
    $sth->bindParam("taskID",$args['taskID']);
    $sth->bindParam("taskRating",$input['taskRating']);
    $sth->bindParam("taskAward",$input['taskAward']);
...
$app->get('/children/{id}/avg-rating', function($request, $response, $args){
    ...
    $sth = $this->db->prepare("select avg(TD.taskRating) From TaskDetails TD inner join Tasks T on T.taskID = TD.taskID where T.assigneeID = :id and TD.taskRating > 0;");
    $sth->bindParam("id", $args['id']);
    $sth->execute();
    $userInfo = $sth->fetchObject();
    return $this->response->withJson($userInfo);
...
$app->get('/getTaskAmount/{id}', function($request, $response, $args){
    ...
    $sth = $this->db->prepare("SELECT count(taskID) as NumTasks FROM Tasks WHERE assigneeID=:id");
    $sth->bindParam("id", $args['id']);
    $sth->execute();
    $userInfo = $sth->fetchObject();
    return $this->response->withJson($userInfo);
